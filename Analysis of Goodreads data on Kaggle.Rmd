...
ggplot(book_language, aes(x=language,y=frequency, fill = factor(c(1:nrow(book_language))))) +
  theme(legend.position = "none") + 
  geom_bar(stat="identity") +
...
ggplot(data=book_ratings_count_top10, aes(x=reorder(title, ratings_count),y=ratings_count, fill = factor(c(1:nrow(book_ratings_count_top10))))) +
  ...
  ggtitle('Books/series with highest number of written ratings') + 
...
```{r}
book_high_ratings <- goodreads[goodreads$average_rating >= 4.3,]
author_most_books_high_ratings <- data.frame(table(book_high_ratings$authors), stringsAsFactors = FALSE)
names(author_most_books_high_ratings) <- c('author','number_of_books_high_ratings')
author_most_books_high_ratings <- author_most_books_high_ratings[order(-author_most_books_high_ratings$number_of_books_high_ratings),] 
head(author_most_books_high_ratings)

author_most_books_high_ratings_top10 <- author_most_books_high_ratings[c(1:10),]
ggplot(data=author_most_books_high_ratings_top10, aes(x=reorder(author, number_of_books_high_ratings),y=number_of_books_high_ratings, fill = factor(c(1:nrow(author_most_books_high_ratings_top10))))) +
  theme(legend.position = "none") + 
  geom_bar(stat="identity") +
  coord_flip() +
  scale_fill_brewer(palette="Set3") + 
  ...
  labs(x = "Author", y = "Total number of books") +
  geom_text(
    aes(label = number_of_books_high_ratings), colour = "black",
    hjust = -0.5, size = 3,
    position = position_dodge(width = 1),
    inherit.aes = TRUE
  )
...
ggplot(data = goodreads, aes(x = goodreads$average_rating)) + 
    geom_histogram(aes(y = ..density..),binwidth = 0.01, color='yellow', fill='yellow') +
    geom_density(colour="red", lwd = 0.5, alpha=0.5) + 
